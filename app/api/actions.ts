  ...
  try {
    ...
    for (const teamRow of teamsResult.rows) {
      ...
      for (const solvedRow of solvedChallengesResult.rows) {
        ...
        const solvedCheckpointIds = solvedCheckpointsResult.rows.map(
        ...
        solvedChallenges.push({
          ...
          solvedCheckpointIds:
            solvedCheckpointIds.length > 0 ? solvedCheckpointIds : undefined,
        });
      }
  ...
  try {
    ...
    for (const challengeRow of challengesResult.rows) {
      ...
      challenges.push({
        ...
        checkpoints:
          checkpointsResult.rows.length > 0
...
export async function deleteTeam(teamId: string) {
  ...
  try {
    ...
    try {
      ...
      await client.query("DELETE FROM solved_checkpoints WHERE team_id = $1", [
        teamId,
      ]);
      await client.query("DELETE FROM solved_challenges WHERE team_id = $1", [
        teamId,
      ]);
      await client.query("DELETE FROM submissions WHERE team_id = $1", [
        teamId,
      ]);
...
export async function addChallenge(formData: FormData) {
  ...
  try {
    ...
    const penaltyPoints =
      Number.parseInt(formData.get("penaltyPoints") as string) || 0;
    ...
    try {
      ...
      if (type === "interactive") {
        const checkpointsCount = Number.parseInt(
          formData.get("checkpointsCount") as string
        );

        for (let i = 0; i < checkpointsCount; i++) {
          const checkpointName = formData.get(`checkpoint_${i}_name`) as string;
          const checkpointPoints = Number.parseInt(
            formData.get(`checkpoint_${i}_points`) as string
          );
          ...
          await client.query(
            ...
            [checkpointId, id, checkpointName, checkpointPoints]
          );
        }
      }
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Failed to add challenge",
    };
  } finally {
    client.release();
  }
...
export async function updateChallenge(challengeId: string, formData: FormData) {
  ...
  try {
    ...
    try {
      ...
      await client.query("DELETE FROM checkpoints WHERE challenge_id = $1", [
        challengeId,
      ]);
      ...
      if (type === "interactive") {
        ...
        for (let i = 0; i < checkpointsCount; i++) {
          ...
          await client.query(
            "INSERT INTO checkpoints (id, challenge_id, name, points) VALUES ($1, $2, $3, $4)",
            [checkpointId, challengeId, checkpointName, checkpointPoints]
          );
        }
      }
...
export async function deleteChallenge(challengeId: string) {
  ...
  try {
    ...
    try {
      ...
      await client.query("DELETE FROM submissions WHERE challenge_id = $1", [
        challengeId,
      ]);
...
export async function addSubmission(
  ...
  try {
    ...
    try {
      ...
      await client.query(
        "INSERT INTO submissions (team_id, challenge_id, is_correct) VALUES ($1, $2, $3, $4)",
        [teamId, challengeId, isCorrect]
      );
...
export async function markNonInteractiveSolved(
  teamId: string,
  challengeId: string
) {
  const client = await pool.connect();
...
export async function markCheckpointsSolved(
  teamId: string,
  challengeId: string,
  ...
  try {
    ...
    try {
      ...
      const currentlySolvedIds = solvedCheckpointsResult.rows.map(
        (row) => row.checkpoint_id
      );
      ...
      for (const checkpointId of currentlySolvedIds) {
        if (!checkpointIds.includes(checkpointId)) {
          await client.query(
            "DELETE FROM solved_checkpoints WHERE team_id = $1 AND checkpoint_id = $2",
            [teamId, checkpointId]
          );
        }
      }
...
export async function getSubmissions(
  ...
  try {
    ...
    let query =
      "SELECT id, team_id, challenge_id, is_correct, submitted_at FROM submissions";
    const params: string[] = [];
...
`);
    ...
    for (const row of result.rows) {
      ...
      const penaltyPoints = row.is_solved
        ? row.wrong_submissions * row.penalty_points
        : 0;
...
export async function calculateTeamPenaltyPoints(
  teamId: string
): Promise<number> {
  const client = await pool.connect();
  ...
  try {
    ...
    for (const row of result.rows) {
      totalPenalty +=
        row.penalty_points * Number.parseInt(row.wrong_submissions);
    }
    ...
    for (const team of teams) {
      ...
      for (const solvedChallenge of team.solvedChallenges) {
        ...
        if (challenge.type === "non-interactive") {
          ...
          score += solvedChallenge.solvedCheckpointIds.reduce(
            (total, checkpointId) => {
  ...
  try {
    ...
    return challenges.map((challenge) => {
      ...
      const completionPercentage =
        totalTeams > 0 ? Math.round((solvedCount / totalTeams) * 100) : 0;
